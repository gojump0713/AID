...
├─ regions.html            # 권역별 선정 현황 (지도 + 대학 카드)
├─ vite.config.js          # 멀티페이지(main/regions) 설정
├─ package.json
├─ public/
│  └─ favicon.svg          # 브랜드 네트워크/허브 파비콘
├─ src/
│  ├─ home.js              # 메인(사업소개) 엔트리
│  ├─ main.js              # 권역 현황 페이지 엔트리
│  ├─ map.js               # 인터랙티브 권역 지도(SVG+JS)
│  ├─ hero.js              # Hero 캔버스 네트워크 애니메이션
│  ├─ nav.js               # 공통 상단 내비게이션
...
│  │  ├─ base.css          # 리셋 + 아이콘 시스템
│  │  └─ app.css           # 페이지 스타일(지도/탭/카드/내비/소개/hero)
...
- **AID 사업소개**(`index.html`, 메인) — 사업 개요·권역 구성·틸론 솔루션 맵. Hero 네트워크 애니메이션. (PDF 한글 추출 이슈로 엑셀 데이터 기반 구성)
- **권역별 선정 현황**(`regions.html`) — 인터랙티브 한국 지도(호버·선택·글로우 효과) + 권역 탭 + 19개 사업단 카드(비전·추진방향·틸론 관점·영업 파트너사). 출처 클릭 시 새 창으로 자료 검색.
