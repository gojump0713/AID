import{d as a,n as k,i as x}from"./hero-HYbOcRqR.js";const m=[{key:"수도권",short:"수도권",cx:188,cy:152,tiles:["150,92 247,86 268,148 232,206 150,206 119,150"]},{key:"강원",short:"강원",cx:338,cy:174,tiles:["282,92 392,118 408,210 346,246 286,200 276,138"]},{key:"충청",short:"충청",cx:187,cy:298,tiles:["128,236 250,234 262,330 176,356 108,312 110,260"]},{key:"부산·울산·경남",short:"부울경",cx:338,cy:322,tiles:["292,236 398,262 382,360 312,402 282,332 282,266"]},{key:"호남·제주",short:"호남·제주",cx:180,cy:420,tiles:["118,346 250,360 246,456 180,498 120,456 104,390","120,556 200,556 206,598 164,614 118,598"]}],S=(e,s)=>`<polygon class="region__tile" points="${e}" style="--ti:${s}"></polygon>`;function E(e,s,n){const c=t=>{var r;return((r=s.find(d=>d.region===t))==null?void 0:r.universities.length)??0},l=m.map(t=>{const r=c(t.key);return`
      <g class="region" data-region="${t.key}" tabindex="0" role="button"
         aria-label="${t.key} 선정대학 ${r}개 보기" style="--d:${m.indexOf(t)*90}ms">
        ${t.tiles.map((d,f)=>S(d,f)).join("")}
        ...
        <g class="region__label" transform="translate(${t.cx} ${t.cy})">
          ...
          <g transform="translate(0 18)">
            ...
            <text class="region__count" text-anchor="middle" y="3">${r}개 단</text>
          </g>
        </g>
      </g>`}).join("");e.innerHTML=`
  ...
  <header class="hero">
    ...
    <div class="container">
      ...
      <div class="brandline">
        ...
        <span>${i("school")} 선정 ${h}개 사업단 · ${a.regions.length}개 권역</span>
      </div>
    </div>
  </header>
...
`;let _=p;const g=document.querySelector("#tabs");g.innerHTML=[p,...a.regions.map(e=>e.region)].map(e=>{const s=e===p?h:w(e).universities.length;return`<button class="tab" data-region="${e}" type="button">
              <span>${e}</span><b>${s}</b>
            </button>`}).join("");const v=document.querySelector("#panel");function A(e){if(e===p){v.innerHTML=`
      <div class="mappanel__empty">
        ${i("touch_app","icon--primary")}
        <h3>권역을 선택하세요</h3>
        <p>전국 ${a.regions.length}개 권역 · ${h}개 사업단.<br>
        지도의 권역을 클릭하면 상세가 표시됩니다.</p>
        <ul class="legend">
          ${a.regions.map(o=>`<li><span class="dot"></span>${o.region} <b>${o.universities.length}</b></li>`).join("")}
        </ul>
      </div>`;return}const s=w(e),n=s.universities.filter(o=>!o.type.includes("연합")).length,c=s.universities.length-n,l=$(e);v.innerHTML=`
    <div class="mappanel__card">
      ...
      <div class="mappanel__partner">
        ${i("handshake","icon--pos")}
        <span><b>영업 파트너</b> · ${l.join(" · ")}</span>
      </div>`:""}
      <div class="mappanel__stats">
        <div><b>${s.universities.length}</b><span>사업단</span></div>
        <div><b>${n}</b><span>단독형</span></div>
        <div><b>${c}</b><span>연합형</span></div>
      </div>
      <ul class="mappanel__list">
        ${s.universities.map(o=>`
          <li>
            <span class="tag tag--${y(o.type)}">${o.type}</span>
            <span class="mappanel__uname">${o.name}</span>
          </li>`).join("")}
      </ul>
    </div>`}const O=document.querySelector("#cards");function T(e,s){const n=y(e.type),c=$(s);return`
    <article class="ucard ucard--${n}">
      ...
      <div class="ucard__cols">
        <div class="ucol">
          ...
          <ul>${e.directions.map(l=>`<li>${i("check","icon--pos")}<span>${l}</span></li>`).join("")}</ul>
        </div>
        <div class="ucol ucol--tilon">
          <h4>${i("lightbulb","icon--primary")} 틸론 관점 제안</h4>
          <ul>${e.tilon.map(l=>`<li>${i("chevron_right","icon--primary")}<span>${H(l)}</span></li>`).join("")}</ul>
        </div>
      </div>
      ${c.length?`
      <div class="ucard__partner" title="해당 권역 영업 파트너사">
        ${i("handshake","icon--pos")}
        <span><b>영업 파트너</b> · ${c.join(" · ")}</span>
      </div>`:""}
      <a class="ucard__src" href="${q(e)}" target="_blank" rel="noopener noreferrer"
         title="출처 자료를 새 창에서 열기">
        ${i("open_in_new")} 출처 · ${e.source}
      </a>
    </article>`}const D=["iStation","Dstation","DaaS","CAS","GPU Slicing","GPU","Sovereign","VDI","NEW-W","멀티테넌시"];function H(e){let s=e;for(const n of D)s=s.replace(new RegExp(`(${n.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")})`,"g"),"<mark>$1</mark>");return s=s.replace(/★/g,'<span class="star">★</span>'),s}function G(e){const s=e===p?a.regions.flatMap(n=>n.universities.map(c=>({u:c,key:n.region}))):w(e).universities.map(n=>({u:n,key:e}));O.innerHTML=s.map(({u:n,key:c},l)=>`<div class="cardslot" style="--i:${l}">${T(n,c)}</div>`).join("")}const I=E(document.querySelector("#map"),a.regions,e=>{u(_===e?p:e)});function u(e){_=e,g.querySelectorAll(".tab").forEach(s=>s.classList.toggle("is-active",s.dataset.region===e)),I.setActive(e===p?null:e),A(e),G(e)}g.querySelectorAll(".tab").forEach(e=>e.addEventListener("click",()=>u(e.dataset.region)));u(p);x(document.querySelector(".hero"));console.info("[AID] report ready —",a.regions.length,"regions /",h,"universities");
