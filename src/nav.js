...
export function navHTML(active = 'home') {
  const regionsHref = active === 'intro' ? './index.html#regions' : '#regions'
  return `
  <nav class="topnav">
    <div class="container topnav__in">
      ...
      <div class="topnav__links">
        <a href="./intro.html" class="${active === 'intro' ? 'is-active' : ''}">
          ${sym('description')} AID 사업소개
        </a>
        <a href="${regionsHref}" class="${active === 'regions' ? 'is-active' : ''}">
          ${sym('map')} 권역별 선정 현황
        </a>
      </div>
    </div>
